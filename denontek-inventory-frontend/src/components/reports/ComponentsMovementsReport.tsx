...
interface ComponentsMovementsReportProps {
  onBack: () => void;
}

export const ComponentsMovementsReport: React.FC<ComponentsMovementsReportProps> = ({ onBack }) => {
  const { components, getComponentMovementLedger } = useInventory();

  // Date filters: 1st of current month by default, last of current month by default
  ...
  const ledger = selectedComponentId
    ? getComponentMovementLedger(selectedComponentId, fromDate, toDate)
    : [];
    ...
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <div className="bg-white px-4 py-3 border-b border-slate-200/80 flex items-center justify-between sticky top-0 z-10 shadow-xs">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-900 leading-tight">Components Movement</h2>
      ...
      <div className="p-3 bg-white border-b border-slate-100 space-y-3">
        ...
        <div>
          <SearchableSelect
            ...
            onChange={(val) => setSelectedComponentId(val)}
            placeholder="Choose component to view ledger..."
          />
        </div>
      ...
      {selectedComp && (
        <div className="px-3 py-2 bg-slate-100/80 border-b border-slate-200 grid grid-cols-3 gap-2 text-center">
          <div className="bg-white p-2 rounded-xl border border-slate-200/60 shadow-2xs">
            <span className="block text-[10px] uppercase font-bold text-emerald-600">Total In</span>
            <span className="text-xs font-black text-slate-900">+{totalIn}</span>
          </div>
          <div className="bg-white p-2 rounded-xl border border-slate-200/60 shadow-2xs">
            <span className="block text-[10px] uppercase font-bold text-amber-600">Total Out</span>
      ...
      <div className="flex-1 overflow-y-auto p-3">
        {!selectedComp ? (
          <div className="text-center py-12 px-4 text-xs text-slate-400">
            Select a component to display movement history
          </div>
        ) : ledger.length === 0 ? (
          <div className="text-center py-12 px-4">
            <FileSpreadsheet className="w-10 h-10 text-slate-300 mx-auto mb-2" />
            <p className="text-xs font-semibold text-slate-600">No transactions found</p>
            <p className="text-[11px] text-slate-400 mt-0.5">Try widening your date range</p>
          </div>
        ) : (
